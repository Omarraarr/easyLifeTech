  ...
  useEffect(() => {
    const fetchCompanyAttendance = async () => {
      try {
        ...
        const response = await axios.get("https://aa-v2.abdullahkhaled.com/api/v1/attendancelocations?", {
          headers: {
        ...
        <ul className="space-y-3">
          {locations.map((location) => (
            ...
            >
              ...
              <p><strong>Map:</strong> {location.map_location}</p>
              <p><strong>Daily Check In:</strong> {location.attendance_today.check_in_count}</p>
              <p><strong>Daily Check Out:</strong> {location.attendance_today.check_out_count}</p>
              {location.image && (
                <img
